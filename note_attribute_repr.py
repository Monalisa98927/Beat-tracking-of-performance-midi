...
def time_to_time_attributes(time):
    """
    Convert an array of onset_time (o_t) or duration_time (d_t) 
    to factorized (o_t_1s, o_t_100ms, o_t_10ms) or (d_t_1s, d_t_100ms, d_t_10ms).
    """

    def t_to_t_1s(t):
        return t // 100

    def t_t_1s_to_t_100ms(t, t_1s):
        return (t - 100 * t_1s) // 10
    
    def t_to_t_10ms(t):
        return t % 10

    time_1s = t_to_t_1s(time)

    time_100ms = t_t_1s_to_t_100ms(time, time_1s)

    time_10ms = t_to_t_10ms(time)

    return np.stack([time_1s, time_100ms, time_10ms], -1)

def onset_beat_to_onset_beat_attributes(o_b):

    def o_b_to_o_b_bt(o_b):
        return o_b // 60
    
    def o_b_to_o_b_sub(o_b):
        return o_b % 60

    o_b_bt = o_b_to_o_b_bt(o_b)

    o_b_sub = o_b_to_o_b_sub(o_b)

    return np.stack([o_b_bt, o_b_sub], -1)

def duration_beat_to_duration_beat_attributes(d_b):

    def d_b_to_d_b_8bt(d_b):
        return d_b // 480
    
    def d_b_to_d_b_1bt(d_b):
        return (d_b % 480) // 60
    
    def d_b_to_d_b_sub(d_b):
        return (d_b % 480) % 60

    d_b_8bt = d_b_to_d_b_8bt(d_b)

    d_b_1bt = d_b_to_d_b_1bt(d_b)

    d_b_sub = d_b_to_d_b_sub(d_b)

    return np.stack([d_b_8bt, d_b_1bt, d_b_sub], -1)
...
def program_to_program_attributes(program):

    def pro_to_pro_type(pro):
        return pro // 8

    def pro_to_pro_sub(pro):
        return pro % 8

    pro_type = pro_to_pro_type(program)
    pro_sub = pro_to_pro_sub(program)
    return np.stack([pro_type, pro_sub], -1)
...
def encode_note_mat_to_atr_mat(nmat,
    ...
    if musebert_version == 'v1':
        if not with_program:
            ...
            atr_mat = np.zeros((tgt_pad_length, 9), dtype=np.int64)
    elif musebert_version == 'v2':
        atr_mat = np.zeros((tgt_pad_length, 14), dtype=np.int64)
    ...
    if musebert_version == 'v1':
        ...
        atr_mat[0: length, 5: 7] = dur_to_dur_attributes(dur)

        # kun: output row [7: 9]: program_type, program_sub
        if with_program:
            atr_mat[0: length, 7:] = program_to_program_attributes(program)

    elif musebert_version == 'v2':
    
        onset_time, duration_time, pitch, onset_beat, duration_beat = \
                nmat[0: length, 0], nmat[0: length, 1], nmat[0: length, 2], nmat[0: length, 3], nmat[0: length, 4]

        
        # output row [0: 3]: o_t_1s, o_t_100ms, o_t_10ms
        atr_mat[0: length, 0: 3] = time_to_time_attributes(onset_time)

        # output row [3: 6]: d_t_1s, d_t_100ms, d_t_10ms
        atr_mat[0: length, 3: 6] = time_to_time_attributes(duration_time)

        # estimate ep or use the given one
        ep = estimate_ep_by_maxmin(pitch, w) if estimate_ep else ep # estimate ep or use the given one
        # output row [6: 9]: p_hig, p_reg, p_deg
        atr_mat[0: length, 6: 9] = pitch_to_pitch_attributes(pitch, ep)

        # output row [9: 11]: o_b_bt, o_b_sub
        atr_mat[0: length, 9: 11] = onset_beat_to_onset_beat_attributes(onset_beat)

        # output row [11: 14]: d_b_8bt, d_b_1bt, d_b_sub
        atr_mat[0: length, 11: 14] = duration_beat_to_duration_beat_attributes(duration_beat)
...
def program_attributes_to_program(pro_type, pro_sub):

    def pro_type_pro_sub_to_program(pro_type, pro_sub):
        return 8 * pro_type + pro_sub

    program = pro_type_pro_sub_to_program(pro_type, pro_sub)

    return program

def time_attributes_to_time(t_1s, t_100ms, t_10ms):

    def t_1s_t_100ms_t_10ms_to_time(t_1s, t_100ms, t_10ms):
        return t_1s * 100 + t_100ms * 10 + t_10ms

    time = t_1s_t_100ms_t_10ms_to_time(t_1s, t_100ms, t_10ms)

    return time

def onset_beat_attributes_to_onset_beat(o_b_bt, o_b_sub):

    def o_b_bt_o_b_sub_to_onset_beat(o_b_bt, o_b_sub):
        return o_b_bt * 60 + o_b_sub

    onset_beat = o_b_bt_o_b_sub_to_onset_beat(o_b_bt, o_b_sub)

    return onset_beat

def duration_beat_attributes_to_duration_beat(d_b_8bt, d_b_1bt, d_b_sub):

    def d_b_8bt_d_b_1bt_d_b_sub_to_duration_beat(d_b_8bt, d_b_1bt, d_b_sub):
        return d_b_8bt * 480 + d_b_1bt * 60 + d_b_sub

    duration_beat = d_b_8bt_d_b_1bt_d_b_sub_to_duration_beat(d_b_8bt, d_b_1bt, d_b_sub)

    return duration_beat
...
def decode_atr_mat_to_nmat(atr_mat, length=None, tgt_pad_length=None):
    ...
    if musebert_version == 'v1':
        ...
        nmat = np.zeros((tgt_pad_length, 5), dtype=np.int64)
    ...
    if musebert_version == 'v1':
        if not with_program:
            ...
            onset_bt, onset_sub, pitch_hig, pitch_reg, pitch_deg, dur_hlf, dur_sqv, prog_type, prog_sub = \
                (atr_mat[0: length, i] for i in range(atr_mat.shape[1]))
        ...
        # kun: row 3: program
        if with_program:
            nmat[0: length, 3] = program_attributes_to_program(prog_type, prog_sub)

    elif musebert_version == 'v2':
        o_t_1s, o_t_100ms, o_t_10ms, d_t_1s, d_t_100ms, d_t_10ms, pitch_hig, pitch_reg, pitch_deg, o_b_bt, o_b_sub, d_b_8bt, d_b_1bt, d_b_sub = \
            (atr_mat[0: length, i] for i in range(atr_mat.shape[1]))

        # row 0: onset time
        nmat[0: length, 0] = time_attributes_to_time(o_t_1s, o_t_100ms, o_t_10ms)

        # row 1: duration time
        nmat[0: length, 1] = time_attributes_to_time(d_t_1s, d_t_100ms, d_t_10ms)

        # row 2: pitch
        nmat[0: length, 2] = pitch_attributes_to_pitch(pitch_hig, pitch_reg, pitch_deg)

        # row 3: onset beat
        nmat[0: length, 3] = onset_beat_attributes_to_onset_beat(o_b_bt, o_b_sub)

        # row 4: duration beat
        nmat[0: length, 4] = duration_beat_attributes_to_duration_beat(d_b_8bt, d_b_1bt, d_b_sub)
